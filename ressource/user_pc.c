...
int minmax_ab( struct config *conf, int mode, int niv, int min, int max, int k );
...
/* Quelques fonctions d'estimation pour évaluer une configuration non terminale */
/* Choisir une fonction d'estimation ou combiner plusieurs ... */
...
{
    int x = estim((struct config *)a), y = estim((struct config *)b);
    if ( x < y )
	return -1;
    if ( x == y )
	return 0;
...
int minmax_ab( struct config *conf, int mode, int niv, int alpha, int beta, int k )
{
 	...
 	struct config T[100];
	   ...
	   if ( k != +INFINI ) {
	   	qsort(T, n, sizeof(struct config), confcmp321); // tri - ordre decroissant
	   	if ( k < n ) n = k; 
	   }

	   score = alpha;
	   for ( i=0; i<n; i++ ) {
   	    	score2 = minmax_ab( &T[i], MIN, niv-1, score, beta, k );
	   ...
	   generer_succ( conf, MIN, T, &n );
    	   ...
    	   if ( k != +INFINI ) {
	    	qsort(T, n, sizeof(struct config), confcmp123); // tri - ordre croissant
	   	if (k < n ) n = k;
	   }

	   score = beta;
	   for ( i=0; i<n; i++ ) {
   	    	score2 = minmax_ab( &T[i], MAX, niv-1, alpha, score, k );
		if (score2 < score) score = score2;
		if (score <= alpha) {
			// Coupe Alpha
   	      		return alpha;   
